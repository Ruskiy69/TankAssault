...
CGL_Display::CGL_Display(const int w, const int h, const char* title, const char* icon): width(w), height(h), fs(false)
{
    ...
    if(icon)
        SDL_WM_SetIcon(gk_sdl::LoadImage_Alpha(icon), NULL);
}
